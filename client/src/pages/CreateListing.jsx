...
export default function CreateListing() {
    ...
    const handleImageSubmit = (e) => {
        if (files.length > 0 && files.length + formData.imageUrls.length < 7) {
            ...
            setImageUploadError('Please select images (max 6)');
            setUploading(false);
        }
    } 
    ...
    <main className='p-3 max-w-4xl mx-auto'>
        ...
        <form className='flex flex-col sm:flex-row gap-4'>
            <div className="flex flex-col gap-4 flex-1">
                ...
                <div className='flex flex-wrap gap-6'>
                    <div className="flex items-center gap-2">
                        <input type="number" id="beds" min='1' max='10' required className="p-3 border border-gray-300 rounded-lg"/>
                        <p>Beds</p>
                    </div>
                    <div className="flex items-center gap-2">
            ...
            <div className='flex flex-col flex-1 gap-4'>
                ...
                image will be the cover (max 6)</span>
                </p>
                ...
                <p className='text-red-700 text-sm'>{ imageUploadError && imageUploadError}</p>
                {
                    formData.imageUrls.length > 0 && formData.imageUrls.map((url, index) => (
                        <div key={url} className='flex justify-between p-3 border items-center'>
                            ...
                            className='w-20 h-20 object-contains rounded-lg'/>
